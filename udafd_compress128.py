...
class UDAFDConfig:
    ...
    use_range_compressor: bool = True
    compressed_bins: int = 128
    compressor_hidden_channels: int = 32
    compressor_kernel_size: int = 5
    compressor_use_skip: bool = True
...
class RangeCompressor(nn.Module):
    """
    Learnable front-end that compresses the range-cell dimension M_raw -> M_comp.
    It only operates along the range dimension and keeps the temporal axis untouched,
    so the GP prior and the LSTM modeling over time remain conceptually valid.
    ...
    def __init__(
        ...
        super().__init__()
        if output_bins <= 0:
            raise ValueError(f'output_bins must be positive, got {output_bins}.')
        if input_bins < output_bins:
            raise ValueError(
                f'RangeCompressor expects input_bins >= output_bins, got {input_bins} < {output_bins}.'
            )
        ...
        def gn(channels: int) -> nn.GroupNorm:
            num_groups = 4 if channels >= 4 else 1
        ...
        self.feature_net = nn.Sequential(
            nn.Conv1d(1, mid_channels, kernel_size=kernel_size, padding=pad, bias=False),
            gn(mid_channels),
            nn.GELU(),
            nn.Conv1d(mid_channels, hidden_channels, kernel_size=kernel_size, stride=2, padding=pad, bias=False),
            gn(hidden_channels),
            nn.GELU(),
            nn.Conv1d(hidden_channels, hidden_channels, kernel_size=kernel_size, stride=2, padding=pad, bias=False),
            gn(hidden_channels),
            nn.GELU(),
            nn.Conv1d(hidden_channels, 1, kernel_size=1, bias=True),
            nn.AdaptiveAvgPool1d(output_bins),
        )
        self.skip_avg = nn.AdaptiveAvgPool1d(output_bins)
        self.skip_max = nn.AdaptiveMaxPool1d(output_bins)
        self.skip_scale = nn.Parameter(torch.tensor(0.5, dtype=torch.float32))
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        B, T_, M_ = X.shape
        if M_ != self.input_bins:
            raise ValueError(f'Expected last dim={self.input_bins}, got {M_}.')

        x = X.reshape(B * T_, 1, M_)
        z = self.feature_net(x)

        if self.use_skip:
            skip = 0.5 * (self.skip_avg(x) + self.skip_max(x))
            z = z + self.skip_scale * skip

        z = z.squeeze(1).view(B, T_, self.output_bins)
        z = self.out_norm(z)
        return z
...
def maybe_compress_sequence(
    X: torch.Tensor,
    range_compressor: Optional[RangeCompressor] = None,
) -> torch.Tensor:
...
def _resolve_frontend_and_decoder_dims(
    raw_input_dim: int,
    cfg: UDAFDConfig,
) -> Tuple[Optional[RangeCompressor], int, int]:
    range_compressor: Optional[RangeCompressor] = None
    feature_input_dim = raw_input_dim

    if cfg.use_range_compressor and raw_input_dim > cfg.compressed_bins:
...
def train_framework(
    ...
    When cfg.use_range_compressor=True and M_raw > cfg.compressed_bins,
