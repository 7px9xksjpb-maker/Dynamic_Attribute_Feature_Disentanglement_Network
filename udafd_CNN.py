...
class UDAFDConfig:
    ...
    # New Configs for Compression
    use_compression: bool = True
    compressed_M: int = 128
...
class RangeCompressionCNN(nn.Module):
    """
    1D CNN to compress the spatial/range cell dimension M (e.g., 512 -> 128) 
    while preserving local scattering center relationships.
    """
    def __init__(self, compressed_M: int = 128):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=8, kernel_size=5, stride=2, padding=2),
            nn.ReLU(),
            nn.Conv1d(in_channels=8, out_channels=1, kernel_size=5, stride=2, padding=2),
            nn.ReLU()
        )
        self.pool = nn.AdaptiveAvgPool1d(compressed_M)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Accommodates both [B, T, M] and [B, crop_len, M]
        shape = x.shape
...
def gaussian_nll(X_true: torch.Tensor, mu_x: torch.Tensor, sigma_x: torch.Tensor) -> torch.Tensor:
    ...
    return nll.sum(dim=(1, 2)).mean() ###  <---改动取消了在M上的平均！！！
...
def compute_counterfactual_reg(
    compressor: RangeCompressionCNN,
    ...
    # Decoder reconstructs the RAW 512 dimensions
    mu_x_cf, _ = decoder(F_D, F_A_cf_rep)
    X_cf_crop, _ = crop_subsequence(mu_x_cf, crop_len=cfg.l, random_crop=True)

    # Must compress the generated counterfactual before feeding to encoder
    X_cf_crop_comp = compressor(X_cf_crop) if cfg.use_compression else X_cf_crop

    mu_A_cf_post = attribute_encoder(X_cf_crop_comp)
...
def extract_attribute_feature(
    ...
    X_comp = compressor(X) if cfg.use_compression else X

    if crop_mode == 'center':
        X_sub, _ = crop_subsequence(X_comp, crop_len=cfg.l, random_crop=False)
        return enc_A(X_sub)

    if crop_mode == 'random_avg':
        feats = []
        for _ in range(num_random_crops):
            X_sub, _ = crop_subsequence(X_comp, crop_len=cfg.l, random_crop=True)
...
def train_framework(
    ...
    encoded_dim = cfg.compressed_M if cfg.use_compression else input_dim

    # Initialize Compressor, Encoders (128), and Decoder (512)
    compressor = RangeCompressionCNN(compressed_M=cfg.compressed_M).to(device)
    enc_D = DynamicEncoder(input_dim=encoded_dim, cfg=cfg).to(device)
    enc_A = AttributeEncoder(input_dim=encoded_dim, cfg=cfg).to(device)
    ...
    optimizer_UDAFD = optim.Adam(
        list(compressor.parameters()) + list(enc_D.parameters()) + list(enc_A.parameters()) + list(decoder.parameters()),
        lr=cfg.lr_1,
    )
    ...
    for epoch in range(epochs_phase1):
        ...
        for batch in dataloader:
            ...
            # 1. Compress raw HRRP and mask for Encoders
            if cfg.use_compression:
                X_comp = compressor(batch_X)
                # Max pool the mask to match compressed dimension mapping
                M_comp = F.adaptive_max_pool1d(batch_mask.view(-1, 1, input_dim), cfg.compressed_M).view(batch_X.size(0), batch_X.size(1), cfg.compressed_M)
            else:
                X_comp = batch_X
                M_comp = batch_mask

            X_windows = split_windows(X_comp, cfg)
            M_windows = split_windows(M_comp, cfg)

            X_subseq, starts = crop_subsequence(X_comp, crop_len=cfg.l, random_crop=True)
            M_subseq, _ = crop_subsequence(M_comp, crop_len=cfg.l, starts=starts, random_crop=False)
            ...
            # 2. Decoder reconstructs RAW 512 HRRP
            mu_x, sigma_x = decoder(F_D, F_A_rep)

            # 3. ELBO Loss evaluates against RAW batch_X
    ...
    for epoch in range(epochs_phase2):
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                F_A_feat = extract_attribute_feature(compressor, enc_A, batch_X, cfg=cfg, crop_mode='center')
    ...
    return compressor, enc_D, enc_A, decoder, rec_net